...
async function carregarHorarios() {
    ...
    if (!data || !servicoId) {
        ...
        selectHorario.innerHTML = '<option value="">Selecione primeiro a data e o serviço</option>';
        return;
    }
    
    try {
        const response = await fetch(`${API_URL}/horarios-disponiveis?data=${data}&servico_id=${servicoId}`);
        ...
        if (horarios.length === 0) {
            selectHorario.innerHTML = '<option value="">Nenhum horário disponível nesta data</option>';
            return;
        }
...
async function buscarHistorico() {
    ...
    try {
        const response = await fetch(`${API_URL}/historico/${telefone}`);
        const agendamentos = await response.json();
        
        const historicoLista = document.getElementById('historicoLista');
        
        if (agendamentos.length === 0) {
            historicoLista.innerHTML = '<p style="text-align: center; color: #666;">Nenhum agendamento encontrado</p>';
            return;
        }
        ...
        historicoLista.innerHTML = agendamentos.map(ag => `
            <div class="card-agendamento ${ag.status === 'cancelado' ? 'cancelado' : ''}">
                ...
                <p><strong>Valor:</strong> R$ ${ag.preco.toFixed(2)}</p>
                <p><strong>Status:</strong> 
                    <span class="status ${ag.status}">${ag.status === 'confirmado' ? '✅ Confirmado' : '❌ Cancelado'}</span>
                </p>
                <p><small><strong>Código:</strong> AG${ag.id}</small></p>
                ${ag.status === 'confirmado' && new Date(ag.data) > new Date() ? 
                    `<button onclick="cancelarAgendamento(${ag.id})" class="btn-cancelar">Cancelar Agendamento</button>` : 
                    ''}
            </div>
        `).join('');
    } catch (error) {
